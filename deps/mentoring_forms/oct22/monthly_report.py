...
__DOC_GEN_TEMPLATE_ID__ = 61
__MONTH__ = '10'
__YEAR__ = '2022'
__DEFAULT_SCHOOL_TARGET__ = 4
__DEFAULT_CLASS_TARGET__ = 10
...
def create_pdf_request(**context):
    ...
    # if dag run month and year is same as form month and year then skip
    if dt_string.month == int(__MONTH__) and dt_string.year == int(__MONTH__):
        return

    report_date = dt_string - relativedelta(months=1)
    query = """
        SELECT * FROM "{}"
        WHERE EXTRACT(MONTH FROM "_CREATION_DATE") = {} 
        and EXTRACT(YEAR FROM "_CREATION_DATE") = {} 
    ...
    for username, data in mentors_data_groups:
        ...
        total_classes_covered = sum(
            int(x['SCHOOL_VISIT_TOTALCLASSES']) for x in data)
        ...
        pdf_data = {
            ...
            'sch_compliance': f"{int((len(data) / school_target) * 100)}%",
            'total_covered': total_classes_covered,
            'class_compliance': f"{int((total_classes_covered / class_target) * 100)}%",
        }
        ...
        if status_code == 200:
            ...
            resp = update_mentor_pdf_url(
                response['data'], username, datetime(year=int(__YEAR__), month=int(__MONTH__), day=1) + relativedelta(day=31))
            if resp == "success":
