...
__table_name_1__ = 'LM2_SEPT23_CORE'
__table_name_2__ = 'LM3_OCT23_CORE'
...
def update_mentor_report_flag(uris, cur):
    query = """
        UPDATE "{}" SET {}='{}'
        WHERE "_URI" IN ({})
        """.format(__table_name_1__, __col_name__, 'pdf generated', ','.join(list(map(lambda x: f"'{x}'", uris))))
    cur.execute(query)
    query = """
        UPDATE "{}" SET {}='{}'
        WHERE "_URI" IN ({})
        """.format(__table_name_2__, __col_name__, 'pdf generated', ','.join(list(map(lambda x: f"'{x}'", uris))))
    cur.execute(query)
...
def create_pdf_request(**context):
    ...
    query = """
        ...
        AND "{}" IS NULL""".format(__table_name_1__, __MONTH__, __YEAR__, __col_name__)
    cur.execute(query)
    ...
    query = """
        ...
        and EXTRACT(YEAR FROM "SCHOOL_VISIT_VISIT_DATE") = {} 
        and "_MARKED_AS_COMPLETE_DATE" IS NOT NULL 
        AND "{}" IS NULL""".format(__table_name_2__, __MONTH__, __YEAR__, __col_name__)
    cur.execute(query)
    mentors2 = cur.fetchall()

    mentors.extend(mentors2)

    logging.info(
        f"Total no of records present in {__table_name_1__} + {__table_name_2__} table on {dt_string}: {len(mentors)}")
    if not mentors:
        return
        ...
        for dataitem in data:
            ...
            if dataitem['CLASS1_DETAILS_GRADE1_PHY']:
                pdf_data.update({
                    ...
                    f'feedback{index}': validate_data(dataitem['CLASS1_DTLS_CLSS1_TECHR_FD_PHY_FEEDBACKDETAILS11_PHY_IMPROVMENT']),
                })
                index += 1
            ...
            if dataitem['CLASS2_DETAILS_GRADE2_PHY']:
                pdf_data.update({
                    ...
                    f'feedback{index}': validate_data(dataitem['CLASS2_DTLS_CLSS2_TECHR_FD_PHY_FEEDBACKDETAILS21_PHY_IMPROVMENT']),
                })
                index += 1
            ...
            if dataitem['CLASS3_DETAILS_GRADE3_PHY']:
                pdf_data.update({
                    ...
                    f'feedback{index}': validate_data(dataitem['CLASS3_DTLS_CLSS3_TECHR_FD_PHY_FEEDBACKDETAILS31_PHY_IMPROVMENT']),
                })
                index += 1
