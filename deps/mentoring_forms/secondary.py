...
__table_name__ = 'SECNDRPHE1_CORE'
__col_name__ = 'daily_report_pdf_status'
headers = {
  'x-hasura-admin-secret': __mentor_report_hasura_secret__,
  'Content-Type': 'application/json'
}
payload = """mutation insert_secondary_school_visit_reports{
      insert_secondary_school_visit_reports(
        objects: [
          {
        ...
        on_conflict: {
          constraint: SECONDARY_SCHOOL_VISIT_REPORTS_pkey,
...
def call_pdf_api(uri):
    query_param = {'id': uri}
    url = __pdf_service_url__ + 'secondary'
...
#         time.sleep(2)
        ...
        if status_code == 200 and response['status'] != 0:
            ...
            logging.info(f"{mentor['OFFICER_DETAILS_USER_NAME']}, {mentor['OFFICER_DETAILS_SCHOOL']}, {report_date}")
