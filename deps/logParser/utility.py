...
LOG_SERVER_URL = "http://68.183.85.218:8000/"
...
def get_log_file():
    ...
    log_url_list = [item.get('href') for item in log_url_list]
    current_date = str((datetime.datetime.now()).date()-datetime.timedelta(days=1)).replace("-", "_")
    today_log_file = "access_"+current_date+".log"
    logging.info(f"Yesterday's log file name is {today_log_file}")
    full_file_path = ""
