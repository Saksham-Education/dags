...
__table_name__ = 'PHY_3_CORE'
__col_name__ = 'MENTOR_REPORT_PDF_STATUS'
headers = {
  'x-hasura-admin-secret': __mentor_report_hasura_secret__,
  'Content-Type': 'application/json'
}
payload = """mutation insert_mentor_monthly_report{
      insert_mentor_monthly_report_pdf(
        objects: [
          {
        ...
        on_conflict: {
          constraint: mentor_monthly_report_pdf_pkey,
...
def call_pdf_api(uri):
    query_param = {'id': uri}
    url = __pdf_service_url__ + 'monthlyVisit'
    r = requests.get(url, params=query_param)
    return r.status_code, r.json()


def update_mentor_pdf_url(pdf_url, username):
    report_date = (datetime.now() - relativedelta(months=1)).strftime("%m-%Y")
    body = payload.replace("%username", username).replace("%report_date", report_date).replace("%url", pdf_url)
...
def update_mentor_report_flag(_uri, cur):
    query = 'UPDATE "{}" SET {}=%s where "_URI"=%s'.format(__table_name__, __col_name__)
...
def create_pdf_request(**context):
    ...
    for mentor in mentors:
        ...
        if status_code == 200 and response['status'] != 0:
            logging.info(
                f"{dt_string}: PDF generated successfully for {response['pdfs'][0]['id']}: Response=> {response['status']}")
            resp = update_mentor_pdf_url(response['pdfs'][0]['url'], response['pdfs'][0]['username'])
            if resp == "success":
