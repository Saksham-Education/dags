...
from deps.logParser import utility_saksham as util
...
def log_parser(**kwargs):
    DATABASE_URL = Variable.get("log_parser_saksham")
...
dag = DAG('log_parser_saksham', default_args=default_args, schedule_interval='0 2 * * *', max_active_runs=1)

start = DummyOperator(task_id="start", dag=dag)
parse_log = PythonOperator(
    dag=dag,
    task_id='log_parser_saksham_task',
